...
if [ $answer == "y" ]; then
    ...
    if [ $answer == "a" ]; then
        ...
        if [ $answer == "y" ]; then
            echo "Removing main.yml file"
            rm $mainyml2
            echo "Please type y if you like to commit this or type a to remove another main.yml ?"
            read answer
            if [ $answer == "y" ]; then
                echo "Adding the commit"
                git add .
            ...
            if [ $answer == "a" ]; then
                echo "Please type the main.yml location for the environment"
                read mainyml3
                cat $mainyml3 | grep -E 'env_name|cloud|aws_region|env_deathdate'
                env_name=`cat $mainyml3 | grep -E 'env_name' | awk '{print $2}'`
                echo ${r}"Please type y if the above information is correct and you want to remove $env_name environment main.yml file PERMANENTLY"${rs}
                read answer
                if [ $answer == "y" ]; then
                    echo "Removing main.yml file"
                    rm $mainyml3
                    echo "Please type y if you like to commit this and push it to $ticketnumber ?"
                    read answer
                    if [ $answer == "y" ]; then
                        echo "Adding the commit"
                        git add .
